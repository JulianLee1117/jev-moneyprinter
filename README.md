...
This aggregate-WFE rule/source combination is closed as an opportunity-generating test. Its narrow interpretation improvement is preserved. The next bounded feasibility direction is a fixed cohort of issuer cash self-tenders with explicit odd-lot eligibility and conservative whole-share economics; the first ABUS price screen failed. Source and economic feasibility must precede another model experiment or platform build. See [PLAN.md](PLAN.md).
