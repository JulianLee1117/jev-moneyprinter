...
The current experiment tests **semiconductor capex composition and supplier read-through**. The new `jev_alpha.readthrough` runner compares native Jev decisions with identical-evidence compact chat, preserves probability distributions and uses explicit condition logic with abstention. Original source text is separated from reviewer labels. Its bounded development cap is $2 across model arms. The financial protocol in `research/experiments/read-through-financial-protocol.v1.json` freezes delayed-entry, long-only, costed evaluation requirements; those returns have not yet been measured.

The first source-only comparison completed: Jev matched 17/18 reviewed answers and compact chat 16/18, but the four condition routes showed no advantage from native confidence over categorical Jev outputs. These three selected examples are not a holdout or alpha test. A thirteen-event price-blind census and a prospective Micron prior are saved for the next stage. See `research/experiments/read-through-development-review-2026-09-18.md` for limitations and artifacts.
