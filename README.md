...
This aggregate-WFE rule/source combination is closed as an opportunity-generating test. Its narrow interpretation improvement is preserved. **The objective is trading alpha specifically.** The tender census is archived preparation, not the current flagship. The first earnings pilot has measured historical return diagnostics; it does not support deploying its positive-mean semantic rule. The next research stage is a broad, dated earnings/guidance cohort testing whether Jev semantic features improve delayed net returns beyond numerical, price and conventional-model controls. This remains an unproven hypothesis; see [PLAN.md](PLAN.md).
