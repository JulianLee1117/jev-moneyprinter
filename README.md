...
**Project paused by user direction, September 19, 2026.** Experiment records, archives and reusable code are preserved. No further development, experiments, broad search allocation or source/venue expansion is authorized by the earlier plans. Reopening requires a concrete, independently checkable economic opportunity and a specific reason that inference cost, coverage or latency prevents simpler methods from exploiting it. An available API or Jev's ability to classify content is insufficient. This pause supersedes earlier next steps; see [the project guidance](PLAN.md).
...
The latest completed experiment is **procurement discovery**. Its bounded ten-source census catalogued 5,771 records; independent review of all 50 model candidates and 90 sampled rejects found **zero verified eligible projects**. Jev was faster but proposed an old-contract false positive and no holdout trade. Nano's only calculable holdout trade was also a false positive and lost **$18.07** on the $1,000 account, or **$20.84** under stressed costs. Earlier fill outcomes remain unknown where historical quote-size units could not be verified. **Stop this pilot without expansion or a prospective checkpoint.** See the [completed procurement review](research/experiments/procurement-review-2026-09-19.md) and [aggregate results](research/experiments/procurement-results-2026-09-19.json).

The earlier experiment reached **earnings-event returns**. Twelve issuers were frozen, eight had paired source inputs, and all sixteen model calls completed before price capture. The six eligible companies with complete model outputs all underperformed IWM. In the conditional $1,000 account illustration, native Jev lost about **$94.92**, versus **$79.79** for categorical Jev; the conventional model selected the same trades as native Jev. No alpha is demonstrated. See the [earnings return review](research/experiments/earnings-pilot-review-2026-09-18.md) and [aggregate results](research/experiments/earnings-pilot-results-2026-09-18.json).

The earlier completed experiment tests **semiconductor capex composition and supplier read-through**. Across 13 frozen events, Jev matched 121/139 unambiguous source judgments versus 78/139 for a conventional compact model, and rejected three unsupported aggregate-equipment claims that the control accepted. Native probability routing changed **zero primary decisions** versus categorical Jev. All 13 primary references were negative, so no opportunity-detection sensitivity or profitability is established. See the [closed comparison review](research/experiments/read-through-closed-review-2026-09-18.md) and [aggregate results](research/experiments/read-through-closed-results-2026-09-18.json).

This aggregate-WFE rule/source combination is closed as an opportunity-generating test. Its narrow interpretation improvement is preserved. **The objective is trading alpha specifically.** The tender census is archived preparation. The first earnings pilot does not support deploying its positive-mean semantic rule, and expansion is paused as the Jev-specific flagship: a small feature panel does not establish a need for Jev's speed or cost advantage.

The [opportunity review](research/experiments/jev-coverage-opportunities-2026-09-18.md) motivated the now-completed procurement pilot. Its proposed breadth/speed mechanism was tested against rules, nano and mini; no qualifying opportunity or profitable Jev contribution was established. The source roster remains incomplete, so this is a stop for insufficient supporting evidence rather than a claim about every possible procurement strategy. See [PLAN.md](PLAN.md).

## Procurement experiment: implementation and current limits

The procurement workflow is implemented in `python -m jev_alpha.procurement`. Its [registered protocol](research/experiments/procurement-protocol.v1.json) fixes ten buyer sources, six listed contractors, July–December 2025, a July–September development split, and an October–December holdout. Rules, Jev, GPT-5.4 nano and GPT-5.4 mini receive the same archived evidence. Each model classifies seven categories around an anchored amount: recipient, procurement stage, amount, amount type, new/existing scope, conditions and prior knowledge. Native Jev distributions are retained; the initial financial rule uses categories. Conditions and prior knowledge are descriptive fields, not additional funding or investor-surprise gates.

Implemented safeguards include source-byte archives and explicit extraction failures; dated issuer aliases and financial references; per-amount coverage with unresolved values preserved; a shared **$50 model ledger** with a **$0.25 conservative per-call ceiling**; source-reviewed development selection among 1, 4 and 8 workers; and frozen inputs, profile and signals before event-price collection. Cached calls retain their original cost but cannot supply fresh latency measurements. Review measures useful project detection, wrong recipients, failures, unresolved evidence and review burden; source labels never repair a model's trading decision.

The bounded census, independent source/model comparison, frozen holdout runs, available quote diagnostic and contribution review are complete. Coverage remains incomplete: three source allocations reached their caps, San Antonio returned no events, and missing/scanned documents, imperfect extraction and partial ownership mapping remain explicit. Jev and nano completed 50 required calls each; mini completed 41, retaining nine failed development attempts. All three completed 13 holdout calls. Current downloads do not recover historical publication clocks, so computed returns are association diagnostics. Full-period model account outcomes remain unknown because pre-November quote-size ambiguity prevents a defensible selected fill; no missing outcome became zero. The final contribution verdict is inconclusive and the research decision is no expansion.

The optional prospective command records one bounded snapshot after a promising result and explicit Jev-contribution review. It establishes each source's initial observed-byte baseline on the first substantive successful capture, records later observations with coverage and prior-existence uncertainty, and stops at 30 **reviewed eligible events** or 90 days. It does not run a scheduler, classify new observations, establish first publication or place trades. See the [procurement runbook](research/experiments/procurement-runbook.md) for the actual command order, authoritative artifact versions and review requirements.

## Earlier experiment implementation

The `jev_alpha.readthrough` runner compares native Jev decisions with identical-evidence compact chat, preserves probability distributions and uses explicit condition logic with abstention. Original source text is separated from reviewer labels. The completed comparison used 26 valid requests, no retries and one shared $2 model cap. `jev_alpha.readthrough_results` distinguishes correct support, unsupported claims and lost support from abstention. The original read-through financial protocol remains frozen; returns for that separate experiment have not been measured.
